...
export const IncidentFormModal: React.FC = () => {
  ...
  const [type, setType] = useState<"accident" | "inspection">("inspection");
  ...
  useEffect(() => {
    if (isEditModalOpen && editingRecord) {
      // Map legacy 'ticket' records into 'inspection' with ticket details
      setType((editingRecord.type as any) === "ticket" ? "inspection" : (editingRecord.type as any));
      ...
      if ((editingRecord.type as any) === "ticket") {
  ...
  const handleSubmit = () => {
    if (!formMonthId) {
      alert("Please select a month for this incident");
      return;
    }
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-lg shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 flex items-center justify-between p-6 border-b border-slate-800 bg-slate-900">
          <h2 className="text-xl font-bold text-cyan-400">
            {isEditModalOpen ? "Edit Incident" : "Add New Incident"}
          </h2>
          <button
            onClick={handleClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="flex gap-3">
              {/* Traffic Ticket removed — ticket data is part of Inspection */}
              <button
                onClick={() => {
                  setType("accident");
                  setInjuries([]);
          ...
          <div className="flex gap-3 pt-4 border-t border-slate-800">
            ...
            >
              {isEditModalOpen ? "Update Incident" : "Create Incident"}
            </button>
            <button
              onClick={handleClose}
